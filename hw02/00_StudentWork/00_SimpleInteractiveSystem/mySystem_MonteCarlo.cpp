...
double MONTE_CARLO_SYSTEM::computeArea( ) const
{
	...
	for (int i = 0; i < mNumSamples; i++)
	{
		if(isInsideArea(mX[i], mY[i]))count++;
	}
	area = count / mNumSamples * (mMaxX - mMinX)*(mMaxY - mMinY);
    cout << "Area:" << area << endl;
    return area;
...
double MONTE_CARLO_SYSTEM::getValue(double x) const
{
    ...
    case FUNCTION_EXPONENTIAL_CONSINE:
		y = -4 * pow(2.718281828, -x / 2.0) * cos(x*x);
        break;
    case FUNCTION_COSINE:
		y = -cos(x)*cos(5 * x) + 4 / (1 + x * x);
        break;
